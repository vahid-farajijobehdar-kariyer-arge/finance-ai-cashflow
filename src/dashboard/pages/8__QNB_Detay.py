...
def load_qnb_data():
    ...
    mask = df['bank_name'].str.lower().str.contains('qnb|finans', na=False, regex=True)
    qnb_df = df[mask].copy()
    if len(qnb_df) > 0:
        qnb_df = add_commission_control(qnb_df)
    return qnb_df
...
def display_data(df: pd.DataFrame):
    ...
    st.download_button("📥 CSV İndir", csv, "qnb_data.csv", "text/csv")
    st.dataframe(df, use_container_width=True, height=400)


def main():
    st.set_page_config(page_title="QNB Detay", page_icon="🏦", layout="wide")
    
    # Require authentication
    if not check_password():
        return
    
    st.title("🏦 QNB Finansbank Detay Analizi")
    st.markdown("---")
    ...
    if len(df) == 0:
        st.error("QNB Finansbank verisi bulunamadı!")
        return
