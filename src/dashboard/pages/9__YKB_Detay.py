...
def load_ykb_data():
    ...
    mask = df['bank_name'].str.lower().str.contains('ykb|yapı|yapi|kredi', na=False, regex=True)
    ykb_df = df[mask].copy()
    if len(ykb_df) > 0:
        ykb_df = add_commission_control(ykb_df)
    return ykb_df
...
def display_data(df: pd.DataFrame):
    ...
    st.download_button("📥 CSV İndir", csv, "ykb_data.csv", "text/csv")
    st.dataframe(df, use_container_width=True, height=400)


def main():
    st.set_page_config(page_title="YKB Detay", page_icon="🏦", layout="wide")
    
    # Require authentication
    if not check_password():
        return
    
    st.title("🏦 YKB (Yapı Kredi) Detay Analizi")
    st.markdown("---")
    ...
    if len(df) == 0:
        st.error("YKB verisi bulunamadı!")
        st.info("Lütfen Dosya Yükle sayfasından YKB dosyası yükleyin.")
        return
