...
@st.cache_data
def load_halkbank_data():
    reader = BankFileReader()
    df = reader.read_all_files()
    mask = df['bank_name'].str.lower().str.contains('halk', na=False)
    halk_df = df[mask].copy()
    if len(halk_df) > 0:
        halk_df = add_commission_control(halk_df)
    return halk_df
...
def display_data(df: pd.DataFrame):
    ...
    st.download_button("📥 CSV İndir", csv, "halkbank_data.csv", "text/csv")
...
def main():
    ...
    with st.spinner("Yükleniyor..."):
        df = load_halkbank_data()
    
    if len(df) == 0:
        st.error("Halkbank verisi bulunamadı!")
        return
