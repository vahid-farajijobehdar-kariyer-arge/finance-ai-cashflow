...
@st.cache_data
def load_garanti_data():
    reader = BankFileReader()
    df = reader.read_all_files()
    mask = df['bank_name'].str.lower().str.contains('garanti', na=False)
    garanti_df = df[mask].copy()
    if len(garanti_df) > 0:
        garanti_df = add_commission_control(garanti_df)
    return garanti_df
...
def display_data(df: pd.DataFrame):
    ...
    st.download_button("📥 CSV İndir", csv, "garanti_data.csv", "text/csv")
    st.dataframe(df, use_container_width=True, height=400)


def main():
    st.set_page_config(page_title="Garanti Detay", page_icon="🏦", layout="wide")
    
    # Require authentication
    if not check_password():
        return
    
    st.title("🏦 Garanti BBVA Detay Analizi")
    st.markdown("---")
    
    with st.spinner("Yükleniyor..."):
        df = load_garanti_data()
    
    if len(df) == 0:
        st.error("Garanti verisi bulunamadı!")
        return
